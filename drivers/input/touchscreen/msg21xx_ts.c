...
#define FW_ADDR_MSG21XX_TP   0x26 //TP slave addr

#define DEMO_MODE_PACKET_LENGTH	(8)

#define TP_PRINT

#define DSX_LOG(fmt,arg...)		   printk("[TP:MSTAR]"fmt"\n",##arg)
...
#ifdef __FIRMWARE_AUTO_UPDATE__

static struct work_struct    msg21xx_wq_update_firmware;
//static struct i2c_client     *msg21xx_i2c_client;
//struct device msg21xx_dev;		/* the device structure		*/

static struct msg21xx_ts_data *msg21xx_data;

static unsigned short fm_major_version;  //fw version
static unsigned short fm_minor_version;

static unsigned short major_version;		//TP CURRENT inside fw version
static unsigned short minor_version;


#define UPDATE_FIRMWARE_NAME "T87_HELITAI_V1.2_2.BIN"

static void  msg21xx_firmware_auto_update(struct work_struct *work);
...
{
	pr_info("the reset gpio is %d\n",pdata->reset_gpio);
	pr_info("the hard_reset_delay_ms is %d\n",pdata->hard_reset_delay_ms);
	pr_info("the post_hard_reset_delay_ms is %d\n",pdata->post_hard_reset_delay_ms);
...
unsigned char msg21xx_get_ic_type(struct msg21xx_ts_data *ts_data)
{
	...
	DSX_LOG("%s, ic_type is %x",__func__,ic_type);
...
static int firmware_erase_c33(struct msg21xx_ts_data *ts_data, EMEM_TYPE_t emem_type)
{
	/* stop mcu */
						...
						size_t size)
{
	...
	if (ret == 0)
		...
		dev_err(dev, "firmware_update failed = %d\n", ret);
...
static int drvTP_erase_emem_c33 (struct msg21xx_ts_data *ts_data, EMEM_TYPE_t emem_type )
{
    // stop mcu
    write_reg(ts_data, 0x0F, 0xE6, 0x0001 );
    ...
    if ( emem_type == EMEM_ALL )
    {
        write_reg_8bit(ts_data, 0x16, 0x08, 0x10 ); //mark
    }
...
static int drvTP_read_info_dwiic_c33 ( struct msg21xx_ts_data *ts_data )
{
    //u8 i;
    u8  dwiic_tx_data[5];
  //  u8  dwiic_rx_data[4];
    ...
    write_reg(ts_data,  0x1E, 0x04, 0x7d60 );

    // TP SW reset
    write_reg(ts_data,  0x1E, 0x04, 0x829F );
    mdelay ( 1 );
    dwiic_tx_data[0] = 0x10;
    dwiic_tx_data[1] = 0x0F;
    dwiic_tx_data[2] = 0xE6;
    dwiic_tx_data[3] = 0x00;
    write_i2c_seq(ts_data, SLAVE_I2C_ID_DBBUS, dwiic_tx_data, 4 );   
    mdelay ( 100 );
  ...
  //  dwiic_tx_data[4] = 0x00;	
    ...
    for(reg_data=0;reg_data<128;reg_data++)
    {
    	...
    	write_i2c_seq(ts_data, FW_ADDR_MSG21XX_TP , dwiic_tx_data, 5 );
	
    	mdelay (50 );

    // recive info data
    	read_i2c_seq(ts_data, FW_ADDR_MSG21XX_TP, &g_dwiic_info_data[reg_data*8], 8);
    }
    return ( 1 );
}
static ssize_t firmware_update_c33_zte( struct device *dev, struct device_attribute *attr,
...
struct msg21xx_ts_data *ts_data = dev_get_drvdata(dev);
    ...
    drvTP_read_info_dwiic_c33(ts_data);
   ...
   if ( g_dwiic_info_data[0] == 'M' && g_dwiic_info_data[1] == 'S' && g_dwiic_info_data[2] == 'T' && g_dwiic_info_data[3] == 'A' && g_dwiic_info_data[4] == 'R' && g_dwiic_info_data[5] == 'T' && g_dwiic_info_data[6] == 'P' && g_dwiic_info_data[7] == 'C' )
    {
        // updata FW Version
        //drvTP_info_updata_C33 ( 8, &fw_bin_data[32][8], 5 );

        g_dwiic_info_data[8]=fw_bin_data[32][8];
        g_dwiic_info_data[9]=fw_bin_data[32][9];
        g_dwiic_info_data[10]=fw_bin_data[32][10];
        g_dwiic_info_data[11]=fw_bin_data[32][11];
        // updata life counter
        ...
        write_reg(ts_data,  0x3C, 0xE4, 0x78C5 );
        write_reg(ts_data,  0x1E, 0x04, 0x7d60 );
        // TP SW reset
        write_reg(ts_data,  0x1E, 0x04, 0x829F );

        mdelay ( 1500 );
        pr_info("TP SW reset still busy ******************\n");
        //polling 0x3CE4 is 0x2F43
        do
        {
            reg_data = read_reg(ts_data, 0x3C, 0xE4 );
       ...
       printk("2F43 still busy ******************\n");
        ...
        write_i2c_seq(ts_data, FW_ADDR_MSG21XX_TP , &g_dwiic_info_data[0], 1024 );
    ...
    //erase main
    drvTP_erase_emem_c33 (ts_data, EMEM_MAIN );
	printk("erase main\n");
    mdelay ( 1000 );
  ...
  printk("Program\n");
    switch ( emem_type )
    {
        ...
        case EMEM_INFO:
            ...
            write_reg_8bit(ts_data, 0x1E, 0x04, 0x9F );
            write_reg_8bit(ts_data, 0x1E, 0x05, 0x82 );
    ...
    for ( i = 0; i < 33; i++ ) // total  33 KB : 2 byte per R/W
    {
        ...
        if ( i < 32 )   //emem_main
        {
            if ( i == 31 )
            {
                fw_bin_data[i][1014] = 0x5A; //Fmr_Loader[1014]=0x5A;
                fw_bin_data[i][1015] = 0xA5; //Fmr_Loader[1015]=0xA5;

                for ( j = 0; j < 1016; j++ )
                {
                    //crc_main=Get_CRC(Fmr_Loader[j],crc_main,&crc_tab[0]);
                    crc_main = Get_CRC ( fw_bin_data[i][j], crc_main, &crc_tab[0] );
                }
            }
            else
            {
        ...
        write_i2c_seq(ts_data, FW_ADDR_MSG21XX_TP, fw_bin_data[i], 1024 );
    ...
    if ( ( emem_type == EMEM_ALL ) || ( emem_type == EMEM_MAIN ) )
    {
        // CRC Main from TP
        crc_main_tp = read_reg(ts_data, 0x3C, 0x80 );
        crc_main_tp = ( crc_main_tp << 16 ) | read_reg(ts_data, 0x3C, 0x82 );

        // CRC Info from TP
        crc_info_tp = read_reg(ts_data, 0x3C, 0xA0 );
        crc_info_tp = ( crc_info_tp << 16 ) | read_reg(ts_data, 0x3C, 0xA2 );
    }
...
/*******************************************/
    ...
    write_i2c_seq(ts_data, SLAVE_I2C_ID_DBBUS, dbbus_tx_data, 4 );
    dbbus_tx_data[0] = 0x10;
    dbbus_tx_data[1] = 0x3C;
    ...
    write_i2c_seq(ts_data, SLAVE_I2C_ID_DBBUS, dbbus_tx_data, 3 );
    read_i2c_seq(ts_data, SLAVE_I2C_ID_DBBUS, &dbbus_rx_data[0], 2 );
/*******************************************/
    if ( dbbus_rx_data[0] == 2 )
    {
       		printk("***chip id =0x%x\n",dbbus_rx_data[0]);
                ret = firmware_update_c33_zte ( dev, attr, buf, size, EMEM_MAIN );
           
    }
    else
    {
      		  printk("chip id =0x%x\n",dbbus_rx_data[0]);
         //   ret = firmware_update_c2 ( dev, attr, buf, size );
	...
	if (ret == 0)
		...
		dev_err(dev, "firmware_update ok = %d\n", ret);
...
#ifdef __FIRMWARE_AUTO_UPDATE__
static int fts_GetFWVer(struct msg21xx_ts_data *ts_data,unsigned long * pu32)
{
 ...
 //   dbbus_tx_data[2] = 0x74;
    write_i2c_seq(ts_data,FW_ADDR_MSG21XX_TP, &dbbus_tx_data[0], 3);
    read_i2c_seq(ts_data,FW_ADDR_MSG21XX_TP, &dbbus_rx_data[0], 4);
...
static void  msg21xx_firmware_auto_update(struct work_struct *work)
{
	struct device *dev = &msg21xx_data->client->dev;
	struct msg21xx_ts_data *ts_data =msg21xx_data;//container_of(msg21xx_i2c_client, struct msg21xx_ts_data, client);	
	unsigned long  ulVer;
...

{
	...
	pr_info("++++++ auto update firmware\n");

	ret = request_firmware(&fw, UPDATE_FIRMWARE_NAME, dev);
	if (ret < 0) {
		dev_err(dev, "Request firmware failed - %s (%d)\n",
						UPDATE_FIRMWARE_NAME, ret);
		return ;
	}

	ts_data->pdata->updating_fw = true;

	disable_irq(msg21xx_data->client->irq);

	pr_info("+++++= auto fw name is %s\n",UPDATE_FIRMWARE_NAME);
	...
	dev_info(dev, "New firmware: %d.%d",
			fw_file_major, fw_file_minor);

       fm_major_version = fw->data[0x3077] + (fw->data[0x3076] << 8);//0x7f4d-0x7f50
       fm_minor_version = fw->data[0x3075] + (fw->data[0x3074] << 8);
	ret = fts_GetFWVer(ts_data,&ulVer);
	if(0 > ret) 
	{
      	    printk("%s:FT_GetFWVer failed. \n",__func__);
	ts_data->pdata->updating_fw = false;
			
	    enable_irq(msg21xx_data->client->irq);
		
      	    return ;
	}
  if(((major_version == fm_major_version) &&(minor_version < fm_minor_version))||(major_version == 17523))
  	{
/*******************************************/
...
/*******************************************/
    ...
    {
       		printk("***chip id =0x%x\n",dbbus_rx_data[0]);
                ret = firmware_update_c33_zte ( dev, 0, 0, 0, EMEM_MAIN );
    ...
    else
    {
  	...
  	{
		pr_info("fw is latest version,no need update\n");
  	}
	//ret = firmware_update_c33(dev, attr, buf, size, EMEM_MAIN, true);
	if (ret == 0)
		dev_err(dev, "firmware_update_c33 et = %d\n", ret);
		...
		dev_err(dev, "firmware_update ok = %d\n", ret);
	memset(fw_bin_data, 0, sizeof(fw_bin_data));
	enable_irq(ts_data->client->irq);
	ts_data->pdata->updating_fw = false;
	...
	if (0 == parse_info(ts_data)) {
		...
		if (ts_data->info.count > 0)	{ /* point touch pressed */
			for (i = 0; i < ts_data->info.count; i++) {
				...
				//DSX_LOG("POINT x IS  %d, y is %d,count is %d\n",ts_data->info.point[i].x,ts_data->info.point[i].y,i);
			}
		}
				...
				struct msg21xx_ts_platform_data *pdata)
{
	...
	DSX_LOG("minX= %d,minY= %d,maxX= %d,maxY= %d\n",coords[0],coords[1],coords[2],coords[3]);
			...
			struct msg21xx_ts_platform_data *pdata)
{
	...
	if (!rc)
		...
		pdata->hard_reset_delay_ms = 10;

	DSX_LOG("%s,reset_delay = %d\n",__func__,pdata->hard_reset_delay_ms);
	...
	if (!rc)
		...
		pdata->hard_reset_delay_ms = 10;
	DSX_LOG("%s,post-hard-reset-delay = %d\n",__func__,pdata->hard_reset_delay_ms);
	...
	if (prop) {
		...
		if (rc) {
			...
			DSX_LOG("%s,CANT READ button mapped\n",__func__);
		}
	}
...
//   char *p = page;
//   struct synaptics_rmi4_data *rmi4_data = msg21xx_data; 


	msg21xx_read_firmware_id(msg21xx_data);
   ...
   len += sprintf(buffer_tpd+len, "I2C address: 0x%x\n", 0x26);
    ...
    len += sprintf(buffer_tpd+len, "Firmware version : MAJ=%x MIN=%x\n", msg21xx_data->pdata->fw_version_major, msg21xx_data->pdata->fw_version_minor);
...
static int msg21xx_ts_probe(struct i2c_client *client,
	...
	if (mt_entry)
		{
       	    DSX_LOG("create tsc_id success\n");
		}
	else
	    DSX_LOG("create tsc_id fail\n");

	DSX_LOG("%s client->irq = %d, @%d\n",__func__,client->irq,__LINE__);
	...
	if (ret)
		{
		DSX_LOG("%s Request irq failed ret = %d,,@%d \n",__func__,ret,__LINE__);
		goto err_req_irq;
		}
	...
	msg21xx_data = ts_data;

	INIT_WORK(&msg21xx_wq_update_firmware, msg21xx_firmware_auto_update);


 	schedule_work(&msg21xx_wq_update_firmware);
#endif
