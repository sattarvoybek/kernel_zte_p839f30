...
static int parse_dt(struct device *dev, struct synaptics_dsx_board_data *bdata)
{
	...
	if (retval == -EINVAL)
		...
		printk("%s get pwr_reg_name failed.\n", __func__);
		bdata->pwr_reg_name = "avdd";
	}
	else
	...
	if (retval == -EINVAL)
		...
		printk("%s get bus_reg_name failed.\n", __func__);
		bdata->bus_reg_name = "vdd";
	}
	else
	...
	if (of_property_read_bool(np, "synaptics,power-gpio")) {
		...
		retval = of_property_read_u32(np, "synaptics,power-on-state",
				&value);
		if (retval < 0) {
			printk("%s get power_on_state failed.\n", __func__);
	...
	if (of_property_read_bool(np, "synaptics,power-delay-ms")) {
		retval = of_property_read_u32(np, "synaptics,power-delay-ms",
				&value);
		if (retval < 0) {
			printk("%s get power_delay_ms failed.\n", __func__);
			bdata->power_delay_ms = 60;
		}
		else
	...
	if (of_property_read_bool(np, "synaptics,reset-gpio")) {
		...
		retval = of_property_read_u32(np, "synaptics,reset-on-state",
				&value);
		if (retval < 0) {
			printk("%s get reset_on_state failed.\n", __func__);
		...
		retval = of_property_read_u32(np, "synaptics,reset-active-ms",
				&value);
		if (retval < 0) {
			printk("%s get reset_active_ms failed.\n", __func__);
			bdata->reset_active_ms = 5;
		}
		else
	...
	{
		pr_info("TP:cant find reset gpio\n");
	}
    

	bdata->vdd_io_gpio = of_get_named_gpio_flags(np,
				"synaptics,vdd-io-gpio", 0, NULL);
	if(bdata->vdd_io_gpio < 0)
	{
		pr_info("TP:cant find vdd_io_gpio gpio\n");
	}

	bdata->vcc_dc_gpio = of_get_named_gpio_flags(np,
				"synaptics,vcc-dc-gpio", 0, NULL);
	if(bdata->vcc_dc_gpio < 0)
	{
		pr_info("TP:cant find vcc_dc_gpio gpio\n");
	}

	bdata->disp_debug_en_gpio = of_get_named_gpio_flags(np,
				"synaptics,disp-debug-en-gpio", 0, NULL);
	if(bdata->disp_debug_en_gpio < 0)
	{
		pr_info("TP:cant find disp_debug_en_gpio gpio\n");
	}
	...
	if (of_property_read_bool(np, "synaptics,max-y-for-2d")) {
		retval = of_property_read_u32(np, "synaptics,max-y-for-2d",
				&value);
		if (retval < 0) {
			printk("%s get max_y_for_2d failed.\n", __func__);
		...
		for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
			...
			if (retval != PAGE_SELECT_LEN) {
				dev_err(rmi4_data->pdev->dev.parent,
						"%s@%d: I2C send (%d) retry %d\n",
						__func__, __LINE__,(int)buf[0],retry + 1);
				msleep(20);
			} else {
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
		...
		dev_err(rmi4_data->pdev->dev.parent,
				"%s@%d: I2C read(0x%x)  retry %d\n",
				__func__, __LINE__,addr,retry + 1);
		msleep(20);
	}

	if (retry == SYN_I2C_RETRY_TIMES) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s@%d: I2C read(0x%x) over retry limit\n",
