...
static struct wakeup_source *gesture_ws;//zhangzhao
#endif

#ifdef GTP_ESD_PROTECT	//zhangzhao add for tsc charger check func
#define SWITCH_OFF		0
#define SWITCH_ON		1
#define CHARGE_CHECK_CIRCLE	120*1000      /* jiffy: ms */


extern bool charger_inster_flag;//zhangzhao add for tsc check

//struct i2c_client  *i2c_connect_client;

static struct delayed_work syna_charge_check_work;
static struct workqueue_struct *syna_charge_check_workqueue;
static void syna_charge_check_func(struct work_struct *work);
void syna_charge_check_switch(struct synaptics_rmi4_data *rmi4_data, int on);
...
//   char *p = page;
   ...
   switch (firmware_information[0]){
        case 0x31:
    		len += sprintf(buffer_tpd+len, "synaptics IC type: %s\n", "s2200 series");
    		break;
    	case 0x32:
    		len += sprintf(buffer_tpd+len, "synaptics IC type: %s\n", "s2202 series");
    		break;
    ...
    len += sprintf(buffer_tpd+len, "Firmware version : %x %x\n", firmware_information[2], firmware_information[3]);
					 ...
					 loff_t * off)
{
	...
	int reg, val;
	int i = 0;
	int g_proc_addr, g_proc_val;	
	struct synaptics_rmi4_data *data = exp_data.rmi4_data; 
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%u,power_fla = %u\n",
			rmi4_data->enable_wakeup_gesture,flag_power_shutdown);
}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	unsigned int gesture_keycode;
	unsigned char gesture_keycode_value[10];
	int i;
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture && (!g_is_touchscreen_window)) {
	DSX_CORE("++data_addr %x data38_offset %x,reg = %x\n",data_addr,extra_data->data38_offset,data_addr + extra_data->data38_offset);
		...
		if (retval < 0)
			return 0;
		pr_info("++++gesture code is %x and is \n",detected_gestures);		
		if (detected_gestures == 1){
			DSX_CORE("it is gesture send ok  f11\n");
			input_report_key(rmi4_data->input_dev, KEY_GESTURE_CLICK, 1);
			input_sync(rmi4_data->input_dev);
			input_report_key(rmi4_data->input_dev, KEY_GESTURE_CLICK, 0);
			input_sync(rmi4_data->input_dev);
			rmi4_data->suspend = false;
		}
		else if(detected_gestures == 0x08)
			{
			DSX_CORE("++++it is gesture report o  f11\n");
			input_report_key(rmi4_data->input_dev, KEY_GESTURE_O, 1);
			input_sync(rmi4_data->input_dev);
			input_report_key(rmi4_data->input_dev, KEY_GESTURE_O, 0);
			input_sync(rmi4_data->input_dev);
			rmi4_data->suspend = false;
		...
		retval = synaptics_rmi4_reg_read(rmi4_data,
				...
				gesture_keycode_value,
				(sizeof(gesture_keycode_value)/sizeof(gesture_keycode_value[0])));
		if (retval < 0)
			return 0;
	for(i=0;i<(sizeof(gesture_keycode_value)/sizeof(gesture_keycode_value[0]));i++)
		pr_info("++++222gesture keycode_value, is %x and is\n",gesture_keycode_value[i]);
			{
            		switch(gesture_keycode_value[6])
            			{
            			case 'c':
            				gesture_keycode = KEY_GESTURE_C;
            				break;
            			case 'e':
            				gesture_keycode = KEY_GESTURE_E;				
            				break;
            			case 'm':
            				gesture_keycode = KEY_GESTURE_M;				
            				break;
            			case 's':
            				gesture_keycode = KEY_GESTURE_S;				
            				break;
            			case 'w':
            				gesture_keycode = KEY_GESTURE_W;				
            				break;
            			default:
            				gesture_keycode = KEY_POWER;				
            				break;
            			}
			DSX_CORE("THE gestrure code is %x\n",gesture_keycode);		
			input_report_key(rmi4_data->input_dev, gesture_keycode, 1);
			input_sync(rmi4_data->input_dev);
			input_report_key(rmi4_data->input_dev, gesture_keycode, 0);
			input_sync(rmi4_data->input_dev);
			}
		}
		return 0;
	...
	for (finger = 0; finger < fingers_supported; finger++) {
		...
		if (finger_status) {
			...
			if(((y<480) &&(1 == g_is_touchscreen_window)) ||(0==g_is_touchscreen_window))
			{
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
		...
		if (detected_gestures) {
				DSX_CORE("it is gesture send ok  f12\n");
...
static irqreturn_t synaptics_rmi4_irq(int irq, void *data)
{
	...
	//DSX_CORE("IT IS IRQ COMING\n");
	if (gpio_get_value(bdata->irq_gpio) != bdata->irq_on_state)
		goto exit;
	#ifdef WAKE_LOCK_GESTURE
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
        DSX_CORE("%s:receive a gesture\n",__func__);
	#ifdef WAKE_LOCK_GESTURE	
        __pm_stay_awake(gesture_ws);
	#endif	
        DSX_CORE("%s:close irq source\n",__func__);
		...
		bool enable, bool attn_only)
{
	...
	const struct synaptics_dsx_board_data *bdata =
      ...
      DSX_CORE("Enter %s@%d:enable=%d,attn_only=%d,irq_count = %d\n",__func__,__LINE__,enable,attn_only,count_irq);   
	if (attn_only) {
	...
	if (enable) {
		...
		if (retval < 0)
			{
			DSX_CORE("ENABLE INT REG FAILED");
			return retval;
			}
		...
		unsigned int intr_count)
{
	...
	DSX_CORE("F01query_base= %x,ctrl_base= %x,data_base= %x,cmd_base= %x\n",rmi4_data->f01_query_base_addr,
		rmi4_data->f01_ctrl_base_addr ,rmi4_data->f01_data_base_addr,rmi4_data->f01_cmd_base_addr );
	return 0;
}
		...
		unsigned int intr_count)
{
	...
	DSX_CORE("%s: Function  max x = %d max y = %d\n",
	...
	if (query_0_5.has_query_27) {
		...
		DSX_CORE("%s@%d:f11_wakeup_gesture=%d,\n",__func__,__LINE__,rmi4_data->f11_wakeup_gesture);   
		...
		unsigned int intr_count)
{
	...
	DSX_CORE("%s@%d:f12_wakeup_gesture=%d,\n",__func__,__LINE__,rmi4_data->f12_wakeup_gesture);   
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			case SYNAPTICS_RMI4_F34:
				...
				DSX_CORE("f34 query_base = %x,ctrl_base = %x,data_base = %x\n",rmi4_data->f34_query_base_addr,
					rmi4_data->f34_ctrl_base_addr,rmi4_data->f34_data_base_addr);
				break;
#endif
			}
...
//			rmi4_data->num_of_fingers);
	...
	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture) {
		...
		set_bit(KEY_GESTURE_CLICK, rmi4_data->input_dev->keybit);
		input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_GESTURE_CLICK);
		...
		set_bit(KEY_GESTURE_O, rmi4_data->input_dev->keybit);
...
static int synaptics_rmi4_set_input_dev(struct synaptics_rmi4_data *rmi4_data)
{
	...
	pr_info("tp rread max-x is  %d,max-y is %d\n",rmi4_data->sensor_max_x,rmi4_data->sensor_max_y );
...
static void gesture_detect_work(struct work_struct *work)
{
    ...
    DSX_CORE("%s:synaptics_rmi4 enable irq source\n",__func__); 
    synaptics_rmi4_int_enable(rmi4_data,true);
    
	// read gesture data via i2c 
    synaptics_rmi4_sensor_report(rmi4_data);
    
    DSX_CORE("%s:release geture_ws\n",__func__);
    #ifdef WAKE_LOCK_GESTURE
...
#define TOUCH_PROC_FILE "driver/tsc_id"
...
static void create_tpd_debug_proc_entry(struct synaptics_rmi4_data *data)
{
	tpd_debug_proc_entry =
		proc_create(TOUCH_PROC_FILE, 0644, NULL,&tpd_touch_proc_ops);
	if (tpd_debug_proc_entry) {
		//tpd_debug_proc_entry->proc_fops = &tpd_touch_proc_ops;
		tpd_debug_proc_entry->data = data;
	} else
...
static int tpd_init_tpinfo(struct tpd_classdev_t *cdev)
{
	...
	TPD_DMESG("%s Type1111:0x%x, Partner:0x%x, FwVersion:0x%x, ControlBase:0x%x.\n", __func__, 
		buf[0], buf[1], ((buf[2] << 8) | (buf[3] & 0xff)), ts->f34_ctrl_base_addr);
...
static int tpd_check_fw_for_upgrade(struct tpd_classdev_t *cdev, unsigned char * data, unsigned int size, unsigned int force_upg)
{
	...
	if(force_upg)// || ((chipid_in_chip == chipid_in_file) && (vendorid_in_chip == vendorid_in_file) &&	(fwver_in_chip < fwver_in_file))) 
		{
...
static unsigned char jdi_fw_data[]={
	#include "synaptics_jdi_fw.h"
};
...
//	uint8_t product_jdi_id[PRODUCT_ID_LEN] = {0x44, 0x53, 0x34, 0x20, 0x52, 0x33, 0x2e, 0x39, 0x2e,0x36};
	...
	else if(0x1 == product_info[2])
		{
		TPD_DMESG("%s find jid manufacturer.\n", __func__);
			g_fw_data = jdi_fw_data;
			fw_size = sizeof(jdi_fw_data);
			force_update = 1;

		/*if(0 == compare_product_id(&product_info[11], product_jdi_id)) {
			g_fw_data = jdi_fw_data;
			fw_size = sizeof(jdi_fw_data);
		} else {
			TPD_DMESG("%s cannot find firmware data.\n", __func__);
...
static void tpd_auto_update_fw_work(struct work_struct *work)
{
	...
	//if(0)
	//tpd_auto_update_fw(&tpd_fw_cdev);
...
void syna_charge_check_switch(struct synaptics_rmi4_data *rmi4_data, int on)
{
	...
	//rmi4_data = i2c_get_clientdata(client);
	DSX_CORE("11111+++zzz++++charger check switch %d\n",on);
	if (SWITCH_ON == on) {
		/* switch on charge check  */
			//rmi4_data->charger_check = 1;
			dev_info(rmi4_data->pdev->dev.parent, "charge check started\n");
			queue_delayed_work(syna_charge_check_workqueue,
				&syna_charge_check_work, msecs_to_jiffies(CHARGE_CHECK_CIRCLE));
		DSX_CORE("+++zzz++++charger check switch %d\n",on);
	} else {
		/* switch off charge check */
			//rmi4_data->charger_check = 0;
			dev_info(rmi4_data->pdev->dev.parent, "charge check cancelled\n");
			cancel_delayed_work_sync(&syna_charge_check_work);
	   DSX_CORE("+++zzz++++charger check switch %d\n",on);
	}
}
...
Output:
	None.
*******************************************************/
extern void TP_check_reset_ed_dcs(void);
static void syna_charge_check_func(struct work_struct *work)
{
	s32 ret = -1;
	struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data;
	unsigned char device_ctrl;
	u16 sof_reg,acq_reg,cal_reg;
	static u16 pre_sof_reg=0,pre_acq_reg=0,pre_cal_reg=0;
	//static int count_error= 0;
	//rmi4_data = i2c_get_clientdata(i2c_connect_client);

	if (rmi4_data->suspend)
		{
		dev_info(rmi4_data->pdev->dev.parent, "charger check terminated!\n");
		return;
	}
#ifdef CONFIG_TPD_AUTO_UPDATE_FIRMWARE
	if (!rmi4_data->stay_awake)
	...
	ret = synaptics_rmi4_reg_read(rmi4_data,
			...
			&device_ctrl,
			sizeof(device_ctrl));
	sof_reg = (u16)device_ctrl & 0x00ff;
	DSX_CORE("111sof reg = %x\n",sof_reg);
	ret = synaptics_rmi4_reg_read(rmi4_data,
			0x0401,
			&device_ctrl,
			sizeof(device_ctrl));
	sof_reg |= ((u16)device_ctrl <<8 )& 0xff00;
	DSX_CORE("sof reg = %x\n",sof_reg);
	...
	ret = synaptics_rmi4_reg_read(rmi4_data,
			...
			&device_ctrl,
			sizeof(device_ctrl));
	acq_reg = (u16)device_ctrl & 0x00ff;
	DSX_CORE("111acq reg = %x\n",acq_reg);

	ret = synaptics_rmi4_reg_read(rmi4_data,
			0x0403,
			&device_ctrl,
			sizeof(device_ctrl));
	acq_reg |= ((u16)device_ctrl <<8 )& 0xff00;
	DSX_CORE("acq reg = %x\n",acq_reg);
	...
	ret = synaptics_rmi4_reg_read(rmi4_data,
			...
			&device_ctrl,
			sizeof(device_ctrl));
	cal_reg = (u16)device_ctrl & 0x00ff;
	DSX_CORE("111cal reg = %x\n",cal_reg);

	ret = synaptics_rmi4_reg_read(rmi4_data,
			0x0405,
			&device_ctrl,
			sizeof(device_ctrl));
	cal_reg |= ((u16)device_ctrl <<8 )& 0xff00;
	DSX_CORE("cal reg = %x\n",cal_reg);
	}
	if( (pre_sof_reg ==sof_reg)||(pre_acq_reg ==acq_reg)||(pre_cal_reg ==cal_reg))
		{
		DSX_CORE("READ THE SAME REG ,ERROR HANPPENED ");
		TP_check_reset_ed_dcs();				
	}
	else
		{
		pre_sof_reg = sof_reg;
...
int syna_lcd_read_tp_reg(unsigned short addr,	unsigned char *data,unsigned short len)
{
	int ret = -1;
	struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data;
		
	if (rmi4_data->suspend) {
		dev_info(rmi4_data->pdev->dev.parent, "lcd read tp reg terminated!\n");
		return ret;
	}
#ifdef CONFIG_TPD_AUTO_UPDATE_FIRMWARE
	if (!rmi4_data->stay_awake)
#endif
	{
	ret = synaptics_rmi4_reg_read(rmi4_data,addr,data,sizeof(unsigned char));
	if(ret<0)
		{
		pr_err("read reg 0x%x error\n",addr);
		return ret;
	}
	DSX_CORE("REG 0X%x is %x",addr,*data);
	}
	return ret;

}

EXPORT_SYMBOL(syna_lcd_read_tp_reg);

bool lcd_get_tsc_gesture_state(void)
{
	struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data;
	if(rmi4_data == NULL)
		{
		pr_info("rmi4_data is NULL \n");
		return 0;
	}
...
EXPORT_SYMBOL(lcd_get_tsc_gesture_state);
...
static ssize_t syna_rmi4_wake_gesture_show(struct kobject *kobj, struct kobj_attribute *attr,char *buf)
{
   struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data; 
...
static ssize_t syna_rmi4_wake_gesture_store(struct kobject *kobj, struct kobj_attribute *attr,
			 const char *buf, size_t count)
 {
   struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data; 
	...
	input = input > 0 ? 1 : 0;
		...
		DSX_CORE("11SCAN input is %d",input);
	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture)
		{
		DSX_CORE("22SCAN input is %d",input);
		rmi4_data->enable_wakeup_gesture = input;
		flag_power_shutdown = !input;
		}
	return count;
...
static ssize_t syna_rmi4_glove_show(struct kobject *kobj, struct kobj_attribute *attr,char *buf)
{
   struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data; 
   unsigned char reg;
   synaptics_rmi4_reg_read(rmi4_data,0x040e,&reg,1);

   return snprintf(buf, PAGE_SIZE, "%u\n",reg);
}

static ssize_t syna_rmi4_glove_store(struct kobject *kobj, struct kobj_attribute *attr,
			 const char *buf, size_t count)
 {
   struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data; 
	unsigned int input;
	...
	input = input > 0 ? 1 : 0;
	if(input)
		command = 0x03;//glove enable
	else
		command = 0x02;//glove disable
		
		synaptics_rmi4_reg_write(rmi4_data, 0x040e, &command, 1);

		synaptics_rmi4_reg_read(rmi4_data, 0x040e, &command, 1);
		
		DSX_CORE("sscanf reg:0x%x val:0x%x\n", 0x040e, command );
...
void synaptics_touchscreen_gloved_finger(bool enable)
{
   struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data; 
	unsigned char command = 0x02;
	...
	if(rmi4_data->suspend == true)
		{
		DSX_CORE("tp in suspend mode,cant enable glove mode");
		return;
		}
...
EXPORT_SYMBOL(synaptics_touchscreen_gloved_finger);

static struct kobj_attribute syna_wake_gesture_attribute =
	__ATTR(wake_gesture, 0666,
	syna_rmi4_wake_gesture_show,
	syna_rmi4_wake_gesture_store);

static struct kobj_attribute syna_glove_attribute =
	__ATTR(glove_mode, 0666,
	syna_rmi4_glove_show,
	syna_rmi4_glove_store);

static struct attribute *wake_gesture_attrs[] = {
	&syna_wake_gesture_attribute.attr,
	&syna_glove_attribute.attr,		
...
static struct kobject *syna_wake_gesture_kobj;
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	syna_wake_gesture_kobj = kobject_create_and_add("syna_wake_gesture", NULL);
	if (!syna_wake_gesture_kobj) {
		retval = -EINVAL;
		goto err_wake_gesture;
	}
	/* Create the files associated with this kobject */
	retval = sysfs_create_group(syna_wake_gesture_kobj, &attr_group);
	if (retval) {
		kobject_put(syna_wake_gesture_kobj);
		goto err_wake_gesture;
	}
	
	mt_entry = proc_create("driver/ts_information", 0644, NULL, &tsc_id);
	if (mt_entry)
		{
       	    DSX_CORE("create tsc_id success\n");
		}
	else
	    DSX_CORE("create tsc_id fail\n");

#ifdef WAKE_LOCK_GESTURE
    gesture_ws = wakeup_source_register("autosleep");
	if (!gesture_ws)
        DSX_CORE("%s:get gesture_ws failed",__func__);
#endif
...
#endif
	//tpd_register_fw_class(rmi4_data);  //zhangzhao delete
#endif
	tpd_existed = 1;
	...
	syna_charge_check_switch(rmi4_data,SWITCH_ON);
#endif
...
static int wake_gesture_count =0 ;
static void synaptics_rmi4_enter_fb_suspend(struct synaptics_rmi4_data *rmi4_data)
{
	...
	const struct synaptics_dsx_board_data *bdata =
      ...
      wakup=1;
      suspendtimes++;
	DSX_CORE("Enter %s@%d.stay_awake=%d,enable gesture=%d,full_pm_cycle=%d,suspendtimes=%d,suspend=%d,wake_gesture_count=%d\n",
		__func__,__LINE__,rmi4_data->stay_awake,rmi4_data->enable_wakeup_gesture,
		rmi4_data->full_pm_cycle,suspendtimes,rmi4_data->suspend,wake_gesture_count);

	if (rmi4_data->stay_awake)
		return;

#ifdef GTP_ESD_PROTECT
	syna_charge_check_switch(rmi4_data,SWITCH_OFF);
#endif


	if (rmi4_data->enable_wakeup_gesture) {
		DSX_CORE("ENABLE GETURE FUNC----\n");
		wake_gesture_count++;
		//flag_power_shutdown = 0;
		synaptics_rmi4_wakeup_gesture(rmi4_data, true);
		if(rmi4_data->enable_wakeup_gesture)//for gesture wake up system
...
static void synaptics_rmi4_late_enter_fb_resume(struct synaptics_rmi4_data *rmi4_data)
{
	...
	//X_CORE("Enter %s@%d.stay_awake=%d,enable gesture=%d\n", __func__,__LINE__,rmi4_data->stay_awake,rmi4_data->enable_wakeup_gesture);
       if(0==wakup)
	   	return ;
       resumetimes++;
	DSX_CORE("Enter %s@%d.stay_awake=%d,enable gesture=%d,full_pm_cycle=%d,resumetimes=%d,suspend = %d,wake_gesture_count = %d\n", 
		__func__,__LINE__,rmi4_data->stay_awake,rmi4_data->enable_wakeup_gesture,
		rmi4_data->full_pm_cycle,resumetimes,rmi4_data->suspend,wake_gesture_count);
	...
	if (rmi4_data->enable_wakeup_gesture) {
		DSX_CORE("DISENABLE GETURE FUNC----\n");
		wake_gesture_count--;
				...
				unsigned long event, void *data)
{
	...
	DSX_CORE("Enter %s. event = %lu\n", __func__,event);
	if (evdata && evdata->data && event == FB_EVENT_BLANK &&
		rmi4_data) {
		blank = evdata->data;
		DSX_CORE("Enter %s. event = %lu,blank = %d\n", __func__,event,*blank);
...
#ifdef GTP_ESD_PROTECT
	INIT_DELAYED_WORK(&syna_charge_check_work, syna_charge_check_func);
	syna_charge_check_workqueue = create_workqueue("syna_charge_check");
#endif
