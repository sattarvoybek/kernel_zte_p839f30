...
static void gpio_keys_gpio_work_func(struct work_struct *work)
{
	...
	wake_lock_timeout(&gpio_keyvol, 3*HZ);
...
static int gpio_keys_probe(struct platform_device *pdev)
{
	...
	wake_lock_init(&gpio_keyvol, WAKE_LOCK_SUSPEND,"gpiokey_lock");
...
int gpio_volume_keys_may_clear_in_resume(struct input_dev *dev, int code)
{
	int i;
	int state = 0;
	...
	/*
	struct device *dev = container_of(container_of(ddata, struct device_private, driver_data), struct device, p);
